...
def run_leaderboard_scraper():
    ...
    try:
        ...
        # Use the latest_csv file (full leaderboard) instead of free models
        latest_csv = os.path.join(LLM_LEADERBOARD_DIR, 'llm_leaderboard_latest.csv')
        if not os.path.exists(latest_csv):
            print("No leaderboard CSV file found after running the scraper.")
            return None

        print(f"Using full leaderboard file: {latest_csv}")
        return latest_csv
