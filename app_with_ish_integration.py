...
def parse_model_name(model_name):
    ...
    # For Gemini models, we need to handle both base variant (flash/pro) and reasoning modifier
    variant = None
    base_variant = None
    is_reasoning = False
    ...
    for pattern in base_variant_patterns:
        match = re.search(pattern, normalized)
        if match:
            base_variant = match.group(1)
            break
    
    # Set the variant based on what we found
    if is_reasoning and base_variant:
        # For reasoning models, we'll handle this in canonical name generation
...
def get_canonical_model_name(model_name):
    ...
    # Add reasoning suffix if this is a reasoning model
    if components.get('is_reasoning', False):
...
def is_google_ai_studio_provider(provider_name):
    """Check if a provider is Google AI Studio (not Vertex AI)."""
    if not provider_name:
        return False
    
    provider_lower = provider_name.lower()
    
    # Must contain both "google" and "ai studio" (in any form)
    has_google = 'google' in provider_lower
    has_ai_studio = 'ai studio' in provider_lower or 'ai_studio' in provider_lower
    has_vertex = 'vertex' in provider_lower
    
    # Return True only if it has Google and AI Studio, but not Vertex
    return has_google and has_ai_studio and not has_vertex
...
def has_free_api_provider(model_name, provider_name):
    ...
    # Special handling for Google AI Studio (various formats)
    if is_google_ai_studio_provider(provider_name):
        # Gemini 2.5 Pro is not available in Google AI Studio free tier
    ...
    if request.method == 'POST':
        ...
        if prompt_from_input or is_regeneration:
            try:
                ...
                # Build proper message context with chat history
                if continuation_context:
                    # Use continuation context instead of normal prompt
                    final_prompt = continuation_context
                    # For continuation, use simple message format
                    messages_for_api = [{"role": "user", "content": final_prompt}]
                elif search_results_str:
                    # Build context with chat history (which already includes the current user message)
                    messages_for_api = build_smart_context_messages(current_chat["history"], "", display_name)
                    
                    # Enhance the last user message with search results
                    if messages_for_api and messages_for_api[-1]["role"] == "user":
                        enhanced_prompt = f"""{search_results_str}
...
Please provide a comprehensive answer that incorporates both the current information from the search results and relevant background knowledge."""
                    ...
                    # Normal chat - build smart context with history (which already includes the current user message)
                    messages_for_api = build_smart_context_messages(current_chat["history"], "", display_name)
                
                print(f"--- Built message context: {len(messages_for_api)} messages for {display_name} ---")
                
                # Debug: Print the message structure (first 100 chars of each message)
                for i, msg in enumerate(messages_for_api):
                    content_preview = msg.get("content", "")[:100] + "..." if len(msg.get("content", "")) > 100 else msg.get("content", "")
                    print(f"--- Message {i+1} ({msg.get('role', 'unknown')}): {content_preview} ---")
    ...
    if not best_match:
        ...
        # Special handling for Qwen models
            ...
            if provider_name.lower() == 'cerebras':
                ...
                if '3' in str(model_components['version']) and '32' in str(model_components['size_b']):
                    # Check if this is the reasoning version
                    if "reasoning" in model_name.lower():
                        return "qwen/qwen3-32b"  # Reasoning version
                    else:
                        return "qwen/qwen3-32b-instruct"  # Non-reasoning version
                else:
                    return "SKIP_PROVIDER"
            elif provider_name.lower() != 'g4f':
                # Other providers don't support Qwen models
                return "SKIP_PROVIDER"
            return model_name
...
def call_cerebras_api(model, messages):
    """
    Calls the Cerebras API directly with proper error handling.
    """
    ...
    # The user-facing name might be different from the API name
    cerebras_model_id = PROVIDER_MODEL_MAP.get(model, {}).get('cerebras', model)
    ...
    try:
        print(f"--- Calling Cerebras API with model: {cerebras_model_id} ---")
        ...
        try:
            ...
            if 'choices' in json_response and json_response['choices']:
                ...
                print(f"--- Cerebras API response missing 'choices': {json_response} ---")
                return None
        except json.JSONDecodeError:
            print(f"--- Failed to decode Cerebras JSON response: {response.text} ---")
            return None

    except requests.exceptions.HTTPError as http_err:
        print(f"--- Cerebras API HTTP error: {http_err} - {response.text} ---")
    except requests.exceptions.RequestException as req_err:
        print(f"--- Cerebras API request failed: {req_err} ---")
    
    return None

def call_groq_api(model, messages):
    """Call Groq API directly with message history."""
    try:
...
def call_google_api(model, messages):
    """Call Google AI Studio API with message history."""
    try:
...
async def call_chutes_api(model, messages):
    """Call Chutes AI API with message history."""
    try:
        # Find the provider-specific model ID
