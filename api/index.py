...
def home():
    try:
        ...
        html = '<html><body style="background:#000;color:#0f0;padding:20px;">'
        html = html + '<h1>Chat Interface</h1>'
        html = html + '<p>Template not found: ' + str(e) + '</p>'
        html = html + '<p>Create api/templates/chat.html</p>'
        html = html + '</body></html>'
        return html
...
def roleplay():
    try:
        ...
        html = '<html><body style="background:#000;color:#0f0;padding:20px;">'
        html = html + '<h1>Roleplay Interface</h1>'
        html = html + '<p>Template not found: ' + str(e) + '</p>'
        html = html + '</body></html>'
        return html
...
'
    ...
    if isinstance(char_data, dict):
        prompt = prompt + 'Name: ' + char_data.get('name', 'Unknown') + '
'
        prompt = prompt + 'Description: ' + char_data.get('description', '') + '
'
        prompt = prompt + 'Personality: ' + char_data.get('personality', '') + '
'
        
        if char_data.get('scenario'):
            prompt = prompt + 'Scenario: ' + char_data.get('scenario') + '
'
