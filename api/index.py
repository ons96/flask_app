...
def home():
    try:
        ...
        return '<h1>Chat Interface</h1><p>Error: Template not found</p>'
...
def call_groq(messages, model):
    ...
    response = requests.post('https://api.groq.com/openai/v1/chat/completions', headers={'Authorization': 'Bearer ' + api_key, 'Content-Type': 'application/json'}, json={'model': model, 'messages': messages, 'temperature': 0.7, 'max_tokens': 2048}, timeout=30)
...
def call_openrouter(messages, model):
    ...
    response = requests.post('https://openrouter.ai/api/v1/chat/completions', headers={'Authorization': 'Bearer ' + api_key, 'Content-Type': 'application/json', 'HTTP-Referer': site_url, 'X-Title': 'BlackBerry Chatbot'}, json={'model': model, 'messages': messages, 'temperature': 0.9}, timeout=60)
    response.raise_for_status()
    return response.json()['choices'][0]['message']['content']

def call_custom_api(messages, model, endpoint, api_key):
    response = requests.post(endpoint, headers={'Authorization': 'Bearer ' + api_key, 'Content-Type': 'application/json'}, json={'model': model, 'messages': messages, 'temperature': 0.7}, timeout=60)
    response.raise_for_status()
    return response.json()['choices'][0]['message']['content']
...
def build_character_prompt(character):
    ...
    if isinstance(char_data, dict):
        base += 'Name: ' + char_data.get('name', 'Unknown') + ' '
        base += 'Description: ' + char_data.get('description', '') + ' '
        base += 'Personality: ' + char_data.get('personality', '') + ' '
        if char_data.get('scenario'):
            base += 'Scenario: ' + char_data.get('scenario')
    return base
