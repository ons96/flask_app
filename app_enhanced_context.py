...
def get_available_models_with_provider_counts():
    ...
    1. Start with fastest free model
    2. Add next fastest models that have higher intelligence OR same intelligence with larger context
    3. Add all remaining free models at the bottom
    """
    ...
    # STEP 1: Start with the fastest model (lowest response_time_s)
    priority_models = []
    ...
    if remaining_models:
        ...
        # STEP 2: Iterate through remaining models by speed, adding those that qualify
        models_to_remove = []
    ...
    # Combine: priority models first, then remaining models
    final_models = priority_models + remaining_models
...
def call_cerebras_api(model, messages):
    """Call Cerebras API directly with message history."""
    try:
        ...
        if CEREBRAS_PROVIDER_CLASS:
            ...
            # For Cerebras, don't add cerebras/ prefix - use the model name directly
            final_model_name = cerebras_model
            if cerebras_model.startswith("cerebras/"):
                final_model_name = cerebras_model.replace("cerebras/", "")
            
            print(f"--- Calling Cerebras API with model: {final_model_name} ---")
            
            # Handle /no_think functionality for Qwen 3 32B
            final_messages = messages.copy()
            if "qwen-3-32b" in final_model_name.lower():
                # Check if the model name indicates non-reasoning mode
                display_name = MODEL_DISPLAY_NAME_MAP.get(model, model)
                if "(Reasoning)" not in display_name and "reasoning" not in model.lower():
                    # Non-reasoning mode requested - append /no_think to the last user message
                    if final_messages and final_messages[-1]["role"] == "user":
                        final_messages[-1]["content"] += " /no_think"
                        print("--- Using Qwen 3 32B in non-reasoning mode (/no_think) ---")
                else:
                    print("--- Using Qwen 3 32B in reasoning mode (default) ---")
            ...
            try:
                ...
                model_max_tokens = get_max_tokens_for_model(final_model_name)
            ...
            response = ChatCompletion.create(
                ...
                max_tokens=get_max_tokens_for_model(final_model_name)
            )
