...
MODEL_DISPLAY_NAME_MAP = {
    ...
    # Standardize Llama 3.3 variants
...
def perform_web_search(query, num_results=3):
    """Performs a web search using DuckDuckGo, Gemini grounding, and SerpAPI as last resort."""
    print(f"--- Performing web search for: {query} ---")
    results_str = "Web search results:\n"
    
    # Try DuckDuckGo first (free and unlimited)
    try:
        ...
        with DDGS() as ddgs:
            ...
            if search_results:
                ...
                print("--- DuckDuckGo search complete. ---")
                return results_str
            else:
                print("--- DuckDuckGo returned no results, trying next method. ---")
    except Exception as e:
        print(f"--- DuckDuckGo search error: {e} ---")
    
    # Try Gemini grounding via Google Generative AI (if API key available)
    if GOOGLE_API_KEY:
        try:
            print("--- Attempting Gemini grounding search ---")
    ...
    # Try SerpAPI as last resort (limited to 100 free searches/month)
    if SERPAPI_API_KEY:
        try:
            print("--- Attempting SerpAPI search (limited free tier) ---")
            ...
            if organic:
                results_str = "Web search results (SerpAPI):\n"
    ...
    results_str += "No search results found or search failed.\n"
    return results_str
# --- End Web Search ---
...
def index():
    ...
    # If this is a POST request, get the selected model from the form
    if request.method == 'POST' and 'model' in request.form:
        ...
        if selected_model in available_model_names:
            ...
            print(f"--- Using model from form submission: {default_model} ---")
    # --- End Default Model ---
    ...
    if session['current_chat'] not in chats:
        if chats:
            # Select the most recent chat if current is invalid (by last_modified time)
            latest_chat_id = sorted(chats.keys(), key=lambda k: chats[k].get('last_modified', chats[k].get('created_at', '')), reverse=True)[0]
    ...
    # If this is a POST request with a model selection, update the current chat's model
    if request.method == 'POST' and 'model' in request.form:
        selected_model = request.form.get('model')
        if selected_model in available_model_names:
            current_chat["model"] = selected_model
            session['user_selected_model'] = selected_model
            # Update the last_modified timestamp when model is changed
            current_chat["last_modified"] = datetime.now().isoformat()
            save_chats(chats)  # Save the updated model selection
            print(f"--- POST: Updated current chat model to: {selected_model} ---")
    ...
    if current_model not in available_model_names and available_model_names:
        ...
        save_chats(chats)  # Save the updated model
    ...
    if request.method == 'POST':
        ...
        if selected_model_for_request not in available_model_names and available_model_names:
            ...
            # Save the valid selected model to the session and update current chat model
            ...
            print(f"--- Saved user selected model to session and current chat: {selected_model_for_request} ---")
        ...
        if prompt_to_use:
            ...
            if not api_messages or not any(msg['role'] == 'user' for msg in api_messages):
                ...
                while should_retry and retry_count < max_retries:
                    ...
                    # Define a list of models known to be supported by Cerebras
                    CEREBRAS_SUPPORTED_MODELS = [
                        "llama-3.3-70b", "llama-3.3-70b-instruct", "meta-llama/llama-3.3-70b-instruct",
                        "llama-4-maverick", "llama-4-maverick-17b", "llama-4-maverick-17b-16e-instruct", 
                        "meta-llama/llama-4-maverick-17b-16e-instruct", "meta-llama/llama-4-maverick-17b-128e-instruct"
                    ]
                    
                    # Step 1: Try Direct Cerebras
                    cerebras_model_id = get_provider_model_id(selected_model_for_request, "cerebras")
                    
                    # Check if the model or any of its variants are supported by Cerebras
                    display_name = MODEL_DISPLAY_NAME_MAP.get(selected_model_for_request, selected_model_for_request)
                    is_supported_by_cerebras = (
                        selected_model_for_request in CEREBRAS_TARGET_MODELS or 
                        selected_model_for_request in CEREBRAS_SUPPORTED_MODELS or
                        any(model for model in CEREBRAS_SUPPORTED_MODELS if MODEL_DISPLAY_NAME_MAP.get(model, "") == display_name) or
                        (cerebras_model_id is not None and cerebras_model_id != selected_model_for_request)
                    )
                    
                    should_try_cerebras = is_supported_by_cerebras and CEREBRAS_API_KEY
                    ...
                    if should_try_cerebras:
                        ...
                        # For Llama 3.3 70B models, use a standardized ID
                        if display_name == "Llama 3.3 70B":
                            model_to_use_cerebras = "llama-3.3-70b-instruct"
                        # For Llama 4 Maverick models, use a standardized ID
                        elif display_name == "Llama 4 Maverick":
                            model_to_use_cerebras = "llama-4-maverick-17b-16e-instruct"
                        # Otherwise use the mapped ID if it exists and is not None, or the original selection
                        else:
                    ...
                    # Define a list of models known to be supported by Groq
                    ...
                    # Check if the model or any of its variants are supported by Groq
                    display_name = MODEL_DISPLAY_NAME_MAP.get(selected_model_for_request, selected_model_for_request)
                    is_supported_by_groq = (
                        (groq_model_id is not None and groq_model_id in GROQ_MODELS_CACHE) or 
                        selected_model_for_request in GROQ_MODELS_CACHE or
                        selected_model_for_request in GROQ_SUPPORTED_MODELS or
                        any(model for model in GROQ_SUPPORTED_MODELS if MODEL_DISPLAY_NAME_MAP.get(model, "") == display_name)
                    )
                    
                    should_try_groq = is_supported_by_groq and GROQ_API_KEY
                    ...
                    if response_content is None and should_try_groq:
                        ...
                        # For Llama 3.3 70B models, use a standardized ID
                        if display_name == "Llama 3.3 70B":
                            model_to_use_groq = "meta-llama/llama-3.3-70b-instruct"
                        # For Llama 4 Maverick models, use a standardized ID
                        elif display_name == "Llama 4 Maverick":
                            model_to_use_groq = "meta-llama/llama-4-maverick-17b-16e-instruct"
                        # Otherwise use the mapped ID if it exists and is in the cache, or the original selection if in cache
                        else:
                        ...
                        # Check if model is valid for Groq - either directly in cache or a known supported model
                        is_valid_groq_model = (
                            GROQ_PROVIDER_CLASS and 
                            (model_to_use_groq in GROQ_MODELS_CACHE or 
                             (display_name == "Llama 3.3 70B" or display_name == "Llama 4 Maverick"))
                        )
                        
                        if is_valid_groq_model:
                    ...
                    # Only use g4f as a last resort if all direct API providers failed
                    if response_content is None:
                        print(f"--- All direct API providers failed or were not applicable. Falling back to g4f providers. ---")
                ...
                if response_content is not None:
                    ...
                    if not is_regeneration:
                        ...
                        current_chat["history"].append({"role": "user", "content": prompt_from_input, "timestamp": current_timestamp}) # Use original prompt here
                        # Update the last_modified timestamp for the chat
                        current_chat["last_modified"] = current_timestamp
                    ...
                    if current_chat.get("name") == "New Chat" and any(msg["role"] == "user" for msg in current_chat["history"]):
                     ...
                     if not is_regeneration:
                         current_timestamp = datetime.now().isoformat()
                         current_chat["history"].append({"role": "user", "content": prompt_from_input, "timestamp": current_timestamp}) # Use original prompt
                         # Update the last_modified timestamp for the chat
                         current_chat["last_modified"] = current_timestamp
                     # Add error message as assistant response
                     current_timestamp = datetime.now().isoformat()
                     current_chat["history"].append({"role": "assistant", "content": final_error_message, "model": selected_model_for_request, "provider": provider_used_str, "timestamp": current_timestamp})
                     # Update the last_modified timestamp for the chat
                     current_chat["last_modified"] = current_timestamp
    ...
    for display_name, model_name, provider_count, intel_index, resp_time, context_window in models_with_perf:
        ...
        # Only show intelligence index and response time, not context window
...
def saved_chats():
    chats = load_chats()
    # Sort chats by last_modified date (falling back to created_at if not available), most recent first
    ...
    chats_html = "".join([f'''<div style="margin: 8px 0; padding: 8px; border-bottom: 1px solid #ddd; background-color: #fff; border-radius: 4px;">
                           ...
                           <small style="color:#666">
                             ...
                             <span style="margin-left: 10px;">Created: {datetime.fromisoformat(chat_data.get('created_at', '1970-01-01T00:00:00')).strftime("%b %d, %Y - %I:%M %p")}</span>
                           </small>
...
def load_chat(chat_id):
    ...
    if chat_id in chats:
        ...
        # Update the last_modified timestamp when a chat is loaded
