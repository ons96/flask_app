...
# Model display name mapping to clean up long names
MODEL_DISPLAY_NAME_MAP = {
    "Gemini 2.5 Flash (Reasoning) (AI_Studio)": "Gemini 2.5 Flash",
    "Gemini 2.0 Flash (AI Studio)": "Gemini 2.0 Flash", 
    "Gemini 2.0 Flash Lite (Preview) (AI Studio)": "Gemini 2.0 Flash Lite",
    "Gemini 1.5 Flash (AI Studio)": "Gemini 1.5 Flash",
    "Gemini 1.5 Flash-8B (AI Studio)": "Gemini 1.5 Flash-8B",
...
# --- Dynamic Ordering Helper Functions ---
...
def get_dynamic_sorted_models():
    ...
    if not free_models:
        print("--- [DYNAMIC_SORT] No free models found, using original sort ---")
        return None
...
# --- End Dynamic Ordering Helper Functions ---
...
def index():
    ...
    # Try to use dynamic sorting for free models, fall back to original if needed
    dynamic_sorted = get_dynamic_sorted_models()
    if dynamic_sorted:
        # Prepend free models from dynamic sorting, then add remaining models
        original_model_names = {name for name, count in available_models_sorted_list}
        dynamic_model_names = {name for name, count in dynamic_sorted}
        remaining_models = [(name, count) for name, count in available_models_sorted_list 
                           if name not in dynamic_model_names]
        available_models_sorted_list = dynamic_sorted + remaining_models
        print(f"--- [INDEX] Using dynamic ordering with {len(dynamic_sorted)} free models prioritized ---")
    ...
    # Create model options with cleaned display names
    model_options_html = ''
    seen_display_names = set()
    for model_name, provider_count in available_models_sorted_list:
        ...
        model_options_html += f'<option value="{model_name}" {selected_attr}>{display_name} ({provider_count} providers)</option>'
